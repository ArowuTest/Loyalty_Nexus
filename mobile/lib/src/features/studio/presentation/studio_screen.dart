    ...
    Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child: Text('🖼 AI Image · Generated by Nexus AI',
...
class _ToolDrawerState extends ConsumerState<_ToolDrawer> {
  ...
  Widget _buildBody(_CatConfig cfg, (String, String)? meta, int after) => Column(
    ...
    // ── Result ready banner (Runway-style: stays open after generation) ──
    if (_resultReady) ...[
      GestureDetector(
        onTap: () => setState(() { _resultReady = false; }),
        child: Container(
          width: double.infinity,
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
          decoration: BoxDecoration(
            gradient: const LinearGradient(colors: [Color(0xFF1B4332), Color(0xFF065F46)]),
            borderRadius: BorderRadius.circular(12),
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Generation queued! Check Gallery for your result.',
                style: TextStyle(color: Color(0xFF4ADE80), fontSize: 12, fontWeight: FontWeight.w600)),
              const SizedBox(height: 2),
              Text('Tap to generate something new with ${widget.tool.name}',
                style: const TextStyle(color: Color(0x8022C55E), fontSize: 10)),
            ])),
            const Icon(Icons.refresh_rounded, size: 14, color: Color(0x8022C55E)),
          ]),
        ),
      ),
      const SizedBox(height: 12),
  ...
  Future<void> _doGenerateRaw(Map<String, dynamic> payload) async {
    ...
    try {
      ...
      if (mounted) setState(() { _generating = false; _resultReady = true; });
      widget.onGenerated();
    } catch (e) {
      _stopGenerationProgress();
