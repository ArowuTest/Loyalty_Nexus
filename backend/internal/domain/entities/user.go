...
// SubscriptionFree is the subscription_status every new account starts with.
// users.subscription_status is NOT NULL DEFAULT 'FREE' with a CHECK on the allowed
// values (migration 022); GORM inserts a zero-valued string as ”, which the CHECK
// rejects, so every creation path must set it explicitly.
