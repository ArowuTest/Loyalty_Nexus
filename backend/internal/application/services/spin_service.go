...
func (s *SpinService) PlaySpin(ctx context.Context, msisdn string) (*entities.Transaction, error) {
	...
	maxSpins := s.cfg.GetInt("max_spins_per_user_per_day", 3)
	if spinCount >= maxSpins {
		return nil, fmt.Errorf("daily spin limit reached (max %d)", maxSpins)
	}
	...
	if user.SpinCredits < 1 {
		return nil, fmt.Errorf("insufficient spin credits: cumulative recharge needed")
	}
	...
	err = s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		// Use SELECT FOR UPDATE to prevent race conditions (Financial Integrity Constraint)
		var lockedUser entities.User
		if err := tx.Raw("SELECT * FROM users WHERE id = ? FOR UPDATE", user.ID).Scan(&lockedUser).Error; err != nil {
			return err
		}
		
		if lockedUser.SpinCredits < 1 {
			return fmt.Errorf("spin credit spent in another session")
		}

		// Deduct Credit
		lockedUser.SpinCredits--
		...
		ledgerTx := &entities.Transaction{
			...
			Amount:      prize.Value, // For liability tracking
			CreatedAt:   time.Now(),
			Metadata:    map[string]any{"prize_name": prize.Name, "type": "spin_play"},
		}
		
		if prize.Type == "bonus_points" {
			lockedUser.TotalPoints += prize.Value
			if err := tx.Save(&lockedUser).Error; err != nil {
				return err
			}
		}
		...
		if prize.Type != "try_again" {
			...
			// Trigger fulfillment out-of-band
...
func (s *SpinService) getDailySpinCount(ctx context.Context, msisdn string) (int, error) {
	...
	err := s.db.WithContext(ctx).Table("transactions").
		Where("msisdn = ? AND metadata->>'type' = 'spin_play' AND created_at >= CURRENT_DATE", msisdn).
		Count(&count).Error
...
func (s *SpinService) getCurrentDailyLiability(ctx context.Context) (int64, error) {
	var total int64
	query := "SELECT COALESCE(SUM(amount), 0) FROM transactions WHERE metadata->>'type' = 'spin_play' AND created_at >= CURRENT_DATE"
...
func (s *SpinService) selectPrize(ctx context.Context, forceLowValue bool) (*prizeRow, error) {
	var prizes []prizeRow
	// REQ-5.3.4: Probability Weights from DB
	query := s.db.WithContext(ctx).Table("prize_pool").Where("is_active = ?", true)
	if forceLowValue {
		// Enforce liability cap: only allow low value or try again
		query = query.Where("base_value <= 1000") // e.g. Max 10 pts
	}
