...
func main() {
	...
	for attempt := 1; attempt <= 30; attempt++ {
		...
		if err == nil {
			...
			if sqlDB, sqlErr := db.DB(); sqlErr == nil {
				...
				maxIdle := 10
				maxLife := 300
				if v, e := strconv.Atoi(os.Getenv("DB_MAX_OPEN_CONNS")); e == nil && v > 0 {
					maxOpen = v
				}
				if v, e := strconv.Atoi(os.Getenv("DB_MAX_IDLE_CONNS")); e == nil && v > 0 {
					maxIdle = v
				}
				if v, e := strconv.Atoi(os.Getenv("DB_CONN_MAX_LIFE")); e == nil && v > 0 {
					maxLife = v
				}
	...
	if db != nil {
		...
		// ─── LLM Orchestrator — prompt/session/memory only; Router V2 owns models ───
		tavilyKey := os.Getenv("TAVILY_API_KEY")
		llmOrch := external.NewLLMOrchestrator(usageTracker, chatRepo, rdb, tavilyKey)
		...
		aiStudioOrch.SetProviderDB(aiProviderRepo) // legacy category registry during Router V2 migration
		aiRoutingRepo := persistence.NewAIRoutingRepository(db)
		aiCapacity := services.NewAICapacityController(rdb)
		aiStudioOrch.SetAIRouting(aiRoutingRepo, aiCapacity)
		llmOrch.SetRouteFunc(aiStudioOrch.ChatRoute)
		llmOrch.SetRouteStreamFunc(aiStudioOrch.ChatRouteStream)
		aiModelIntelRepo := persistence.NewAIModelIntelligenceRepository(db)
		aiModelIntelWorker := services.NewAIModelIntelligenceWorker(aiModelIntelRepo, aiProviderRepo, aiStudioOrch)
		aiModelIntelWorker.Start(ctx)
		aiProviderH := handlers.NewAIProviderAdminHandler(aiProviderRepo)
		aiRoutingH := handlers.NewAIRoutingAdminHandler(aiRoutingRepo, aiProviderRepo)
		aiModelIntelH := handlers.NewAIModelIntelligenceAdminHandler(aiModelIntelRepo, aiProviderRepo, aiRoutingRepo)
		...
		studioH.SetAIStudioOrchestrator(aiStudioOrch)
		...
		mux.Handle("POST /api/v1/studio/voice/clone", auth(http.HandlerFunc(studioH.CloneVoice))) // Talking Avatar: register user's ElevenLabs voice clone
		...
		mux.Handle("PUT    /api/v1/admin/prizes/config", adminAuth(http.HandlerFunc(adminH.PublishPrizeConfiguration)))
		...
		// AI Routing V2 — per-tool/per-stage routes and surge policy
		mux.Handle("GET    /api/v1/admin/ai-routing/tools/{slug}", adminAuth(http.HandlerFunc(aiRoutingH.GetToolRoute)))
		mux.Handle("POST   /api/v1/admin/ai-routing/tools/{slug}/validate", adminAuth(http.HandlerFunc(aiRoutingH.ValidateToolRoute)))
		mux.Handle("PUT    /api/v1/admin/ai-routing/stages/{id}", adminAuth(http.HandlerFunc(aiRoutingH.UpdateStage)))
		mux.Handle("POST   /api/v1/admin/ai-routing/stages/{id}/bindings", adminAuth(http.HandlerFunc(aiRoutingH.CreateBinding)))
		mux.Handle("PUT    /api/v1/admin/ai-routing/bindings/{id}", adminAuth(http.HandlerFunc(aiRoutingH.UpdateBinding)))
		mux.Handle("DELETE /api/v1/admin/ai-routing/bindings/{id}", adminAuth(http.HandlerFunc(aiRoutingH.DeleteBinding)))
		// Model Intelligence + AI Scout recommendations
		mux.Handle("GET    /api/v1/admin/ai-models", adminAuth(http.HandlerFunc(aiModelIntelH.ListModels)))
		mux.Handle("GET    /api/v1/admin/ai-model-recommendations", adminAuth(http.HandlerFunc(aiModelIntelH.ListRecommendations)))
		mux.Handle("POST   /api/v1/admin/ai-model-recommendations/{id}/approve", adminAuth(http.HandlerFunc(aiModelIntelH.ApproveRecommendation)))
		mux.Handle("POST   /api/v1/admin/ai-model-recommendations/{id}/reject", adminAuth(http.HandlerFunc(aiModelIntelH.RejectRecommendation)))
