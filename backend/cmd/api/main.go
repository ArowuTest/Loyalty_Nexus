...
func main() {
	...
	spinSvc := services.NewSpinService(userRepo, txRepo, nil, nil, cfg, db) 
	...
	ussdHandler := handlers.NewUSSDHandler(userRepo)
	...
	mux.Handle("/api/v1/admin/config/update", middleware.AuthMiddleware(http.HandlerFunc(adminHandler.UpdateProgramConfig)))
	mux.Handle("/api/v1/admin/prizes", middleware.AuthMiddleware(http.HandlerFunc(adminHandler.ListPrizes)))
	mux.Handle("/api/v1/admin/prizes/update", middleware.AuthMiddleware(http.HandlerFunc(adminHandler.UpdatePrizeWeight)))
	mux.Handle("/api/v1/admin/notifications/template/update", middleware.AuthMiddleware(http.HandlerFunc(adminHandler.UpdateSMSTemplate)))
	mux.Handle("/api/v1/admin/fraud/alerts", middleware.AuthMiddleware(http.HandlerFunc(adminHandler.ListFraudAlerts)))
