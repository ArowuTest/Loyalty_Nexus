-- =============================================================================
-- Migration 121: Ghost-migration recovery for 118-120
-- =============================================================================
--
-- Why this exists:
--   Migrations 118, 119, and 120 were recorded as "applied" in schema_migrations
--   during a deploy that started but whose SQL never fully executed against the
--   live database (stale/ghost rows). golang-migrate skips them on every
--   subsequent deploy because it trusts schema_migrations.
--
--   This migration is version 121 — not yet in schema_migrations — so the runner
--   WILL execute it. It re-applies the SQL from 118, 119, and 120 inline using
--   fully idempotent guards. If those migrations did run correctly, every
--   statement below is a safe no-op. If they were ghost-skipped, this fixes
--   the platform in one deploy.
--
