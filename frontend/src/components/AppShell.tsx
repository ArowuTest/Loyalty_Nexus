...
function getInitials(user: { display_name?: string; phone_number?: string } | null): string {
  ...
  // Use phone carrier prefix (first 4 chars) as fallback — more meaningful than last 2 digits
  const phone = (user.phone_number ?? "").replace(/^\+?234/, "0");
    ...
    <div className="min-h-screen flex flex-col" style={{ background: "var(--surface-0)" }}>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Points balance — desktop (BUG-032 fix) */}
          {wallet && (
            <span className="hidden md:flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-black" style={{ background: `${tierColor}22`, border: `1.5px solid ${tierColor}55`, color: tierColor }}>
              ⚡ {wallet.pulse_points >= 1_000_000 ? `${(wallet.pulse_points / 1_000_000).toFixed(1)}M` : wallet.pulse_points >= 1000 ? `${(wallet.pulse_points / 1000).toFixed(1)}K` : wallet.pulse_points.toLocaleString()} pts
            </span>
          )}
