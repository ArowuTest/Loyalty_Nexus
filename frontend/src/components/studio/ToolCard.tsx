...
import { Lock, Sparkles, ArrowRight, Info, Wand2 } from 'lucide-react';
...
export const ToolCard: React.FC<ToolCardProps> = ({ tool, userPoints, onSelect }) => {
  // Support both snake_case (new) and camelCase (old) field names for backward compat
  const pointCost = tool.point_cost;
  const isLocked = userPoints < pointCost;
  const iconName = tool.icon ?? "Wand2";
  const IconComponent = ((LucideIcons as unknown) as Record<string, React.FC<{ size?: number }>>)[iconName] ?? Wand2;
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        `}>
          ...
          {pointCost === 0 ? 'Free' : `${pointCost} pts/gen`}
        </div>
      </div>
      ...
      <div className="mt-6 pt-4 border-t border-white/5 flex items-center justify-between">
        {isLocked ? (
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
            Need {(pointCost - userPoints).toLocaleString()} more pts
          </p>
        ) : (
