...
export default function StudioLanding() {
  ...
  const [userPoints] = useState(12); // Mock balance for UI demo
  ...
  return (
    <div className="min-h-screen space-y-10 py-12 px-6 max-w-screen-xl mx-auto">
      {/* Header Section */}
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="glass px-6 py-4 rounded-3xl border-brand-gold/30 flex items-center gap-4 shadow-xl shadow-brand-gold/5">
          <div className="text-right">
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Available Balance</p>
            <p className="text-2xl font-black text-white italic">{userPoints} <span className="text-brand-gold">PTS</span></p>
          </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between border-b border-white/5 pb-6">
        <nav className="flex gap-2 p-1 glass rounded-2xl border-white/5 overflow-x-auto w-full md:w-auto no-scrollbar">
          <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <div className="glass rounded-3xl p-5 border border-white/5 opacity-40 flex flex-col justify-center items-center text-center space-y-3 grayscale">
          <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-slate-500">
            <Sparkles size={24} />
          </div>
          <h3 className="text-lg font-bold text-white italic">More Coming...</h3>
